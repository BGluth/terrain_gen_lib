...
pub fn process_reg_info(raw_reg_info: RawRegistrationInfo) -> Result<RegisteredInfo, String>
{
    ...
    validate_that_no_passes_have_both_read_write_on_any_data(&reg_info.pass_data.pass_meta_data.pass_mappings)?;
...
fn validate_that_all_passes_have_key(reg_info: &RawRegistrationInfo, key: PassInfoDataKeys, missing_data_str: &str) -> GenerationError
{
    ...
    {   
        let mut err_str = format!("The following registered pass info do not have a corresponding registered {}:\n", missing_data_str);
...
fn validate_that_no_passes_have_both_read_write_on_any_data(p_mappings: &PassMappings) -> GenerationError
{
    ...
    for (pass_name, data_read_by_pass) in p_mappings.pass_data_read_reqs.iter()
    {
        match p_mappings.pass_data_write_reqs.get(pass_name)
        {
            ...
            {
                let keys_in_both_iter = write_data_keys.iter().filter(|w_key| data_read_by_pass.contains(w_key));
    ...
    {
        let mut err_str = String::from("The following passes have the same datatypes registered as both read and write:\n");
        append_parent_and_child_elements_to_string(data_with_read_and_write.into_iter(), &mut err_str);
...
fn validate_that_all_prereq_passes_of_passes_exist(p_meta: &PassMetaData) -> GenerationError
{
    ...
    {
        let missing_prereqs = prereqs.iter().filter(|prereq| !p_meta.pass_names.contains(prereq));
    ...
    {
        let mut err_str = String::from("The following passes have prerequisite passes that are not defined:");
...
fn validate_that_all_requested_data_is_written_by_a_pass(gen_state: &RegisteredInfo) -> GenerationError
{
    unimplemented!();
}

fn extract_data_from_raw_reg_info_into_gen_state(raw_reg_info: RawRegistrationInfo) -> RegisteredInfo
{
...
fn extract_data_from_pass_info_into_reg_info(mut raw_pass_info: RawPassInfo, p_data: &mut PassData)
{
    p_data.pass_meta_data.pass_names.push(raw_pass_info.name);
    p_data.pass_core_data.run_funcs.insert(raw_pass_info.name, raw_pass_info.run_func);
    ...
    for (key, val) in data_drain
    {
        match key
        {
            PassInfoDataKeys::PassDesc =>
            {
                ...
                p_data.pass_meta_data.pass_descs.insert(&raw_pass_info.name, desc);
            },
        }
    }
    ...
    for (key, val) in grouped_data_drain
    {
        match key
        {
            ...
            { 
                p_data.pass_meta_data.pass_mappings.pass_data_req_params.insert(&raw_pass_info.name, val);
            }

            PassInfoGroupedDataKeys::ReadAcc =>
            {
                p_data.pass_meta_data.pass_mappings.pass_data_read_reqs.insert(raw_pass_info.name, val);
            },

            PassInfoGroupedDataKeys::WriteAcc =>
            {
                p_data.pass_meta_data.pass_mappings.pass_data_write_reqs.insert(raw_pass_info.name, val);
            }

            PassInfoGroupedDataKeys::CreateAcc =>
            {
                p_data.pass_meta_data.pass_mappings.pass_data_create_reqs.insert(raw_pass_info.name, val);
            }

            PassInfoGroupedDataKeys::PrereqPass =>
            {
                p_data.pass_meta_data.pass_mappings.prereq_passes_of_passes.insert(raw_pass_info.name, val);
            }
        }
    }
}

fn append_parent_and_child_elements_to_string<'a, I>(par_str_with_bullets: I, str_to_append_to: &mut String)
