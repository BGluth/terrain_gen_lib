...
use crate::GeneratorState::{PassId, DataId, ParamId, PassRunFunc, PassRunResult};
use crate::GeneratorState::{process_reg_info, RawRegistrationInfo};
use crate::GeneratorState::{RegisteredInfo, PassData, NonPassData, PassCoreData, PassMappings, PassMetaData, ParamsAndData};
...
impl PassThreadState
{
    ...
    fn schedule_run_func_on_thread(&mut self, run_func: &Box<PassRunFunc>, p_data: DataRequestedByPass)
    {
...
impl PassStages
{
    ...
    {
        let ready_pass_id = self.passes_waiting_for_resources.swap_remove(res_waiting_idx);
...
impl PrereqsOfPasses
{
    ...
    fn notify_pass_completed_and_ret_pids_that_can_be_moved_to_waiting_for_created(&mut self, pass_id_completed: PassId) -> Vec<PassId>
    {
        let mut pids_that_have_all_prereqs_met = Vec::new();
        let pid_prereqs_of_comp_pid = self.passes_that_are_prereqs_of_pass.get_mut(pass_id_completed).into_iter().flatten();
...
impl PassesWaitingOnDataToBeCreated
{
    fn new(p_mappings: &PassMappings) -> PassesWaitingOnDataToBeCreated
    {
        ...
        for (pid, dids_waited_on) in p_mappings.pass_data_create_reqs.iter()
        {
            ...
            {
                let waiting_dids = pass_ids_waiting_on_data_id.entry(*did).or_insert(Vec::new());
    ...
    fn notify_data_created_and_ret_pids_that_can_be_moved_to_waiting_for_res(&mut self, did_created: DataId, to_waiting_pids: &mut Vec<PassId>)
    {
        let pids_that_are_waiting_on_did = self.pass_ids_waiting_on_data_id.get(did_created).into_iter().flatten();
...
impl PassRequiredRes
{
    ...
    {
        PassRequiredRes { pid: p_id, did_and_req_types: d_ids }
    }
}
    ...
    fn pass_req_res_are_avail(&self, p_req_res: &PassRequiredRes) -> bool
    {
        for (did, req_type) in p_req_res.did_and_req_types.iter()
        {
            match req_type
            {
                DataReqType::READ => 
                {
                    if self.state[did].write_active == true { return false; }
                },

                DataReqType::WRITE =>
                {
                    if self.state[did].num_readers > 0 { return false; }
                }
            }
        }
...
impl DataUseState
{
    fn new() -> DataUseState
    {
        DataUseState { num_readers: 0, write_active: false }
    }
}
...
fn run_generation_passes(reg_info: RegisteredInfo) -> GenerationError
{
    ...
    while gen_state.sched_state.num_passes_rem > 0
    {
        ...
        upgrade_any_passes_from_waiting_for_res_to_ready_for_sched(&mut gen_state.sched_state.p_stages, &mut gen_state.sched_state.p_res_state);
        schedule_ready_passes_on_avail_threads(&mut gen_state.sched_state.p_stages, &gen_state.sched_state.p_res_state,
            &gen_state.pass_data.pass_core_data.run_funcs, &mut gen_state.pass_data.pass_core_data.params_and_data,
...
fn block_and_processes_comp_msgs_if_no_threads_avail(gen_state: &mut GenerationState) -> GenerationError
{
...
fn block_and_processes_comp_msgs_if_no_passes_avail(gen_state: &mut GenerationState) -> GenerationError
{
    if gen_state.sched_state.p_stages.passes_ready_for_scheduling.len() > 0
    {
        return Ok(());
    }
...
{
    let pass_id = gen_state.thread_state.t_comp_msgs_rx.recv()
        .map_err(|e| conv_err_to_str_with_when(Box::new(e),
...
fn process_thread_completed_msgs(gen_state: &mut GenerationState) -> GenerationError
{
    ...
    {
        let pass_id = gen_state.thread_state.t_comp_msgs_rx.try_recv()
            .map_err(|e| conv_err_to_str_with_when(Box::new(e),
...
{
    let new_pass_ids_with_comp_prereqs = gen_state.sched_state.p_stages.prereqs_of_passes
    ...
    {
        if !gen_state.sched_state.p_stages.passes_waiting_on_data_to_be_created.pass_is_waiting(pid)
        {
    ...
    update_passes_waiting_on_non_existing_data_with_completed_pass(comp_pass_id, &mut gen_state.sched_state.p_stages,
        &gen_state.pass_data.pass_meta_data.pass_mappings);

    let pass_req_res = &gen_state.sched_state.p_res_state.pass_req_res[comp_pass_id];
    gen_state.sched_state.p_res_state.data_usage_state.release_resources_of_completed_pass(pass_req_res);
...
fn get_data_reqs_for_passes(p_core_data: &PassCoreData, p_meta: &PassMetaData) -> HashMap<PassId, PassRequiredRes>
{
    ...
    for pid in p_meta.get_pass_ids()
    {
        ...
        data_req.extend(p_meta.pass_mappings.pass_data_read_reqs.get(pid).into_iter().flat_map(|d_ids| d_ids.into_iter().map(|d_id| (*d_id, DataReqType::READ))));
        data_req.extend(p_meta.pass_mappings.pass_data_write_reqs.get(pid).into_iter().flat_map(|d_ids| d_ids.into_iter().map(|d_id| (*d_id, DataReqType::WRITE))));
...
fn update_passes_waiting_on_non_existing_data_with_completed_pass(comp_pid: PassId, p_stages: &mut PassStages, p_mappings: &PassMappings)
{
    let dids_created_by_pass = p_mappings.pass_data_create_reqs.get(comp_pid).into_iter().flatten();
    
    let mut pids_that_can_go_to_res_waiting = Vec::new();
    for did in dids_created_by_pass
    {
        p_stages.passes_waiting_on_data_to_be_created
            .notify_data_created_and_ret_pids_that_can_be_moved_to_waiting_for_res
            (did, &mut pids_that_can_go_to_res_waiting);
    }    

    p_stages.passes_waiting_for_resources.extend(pids_that_can_go_to_res_waiting);
}

fn schedule_ready_passes_on_avail_threads(p_stages: &mut PassStages, p_res_state: &PassResState,
    run_funcs: &HashMap<PassId, Box<PassRunFunc>>, params_and_data: &mut ParamsAndData,
    p_mappings: &PassMappings, t_state: &mut PassThreadState)
{
    let num_passes_to_schedule = min(t_state.num_threads_avail, p_stages.passes_ready_for_scheduling.len());
...
fn get_data_req_by_pass<'a>(pid: PassId, p_req_res_opt: Option<&'a PassRequiredRes>, p_mappings: &'a PassMappings, params_and_data: &'a mut ParamsAndData) -> DataRequestedByPass<'a>
{
    let mut collected_data = DataRequestedByPass::new();

    add_params_and_setup_req_pass_data_for_any_created_data(pid, &mut collected_data, params_and_data, p_mappings);

    match p_req_res_opt
    {
        Some(p_req_res) => add_read_write_data_to_req_pass_data(p_req_res, &mut collected_data, params_and_data),
        None => ()
    }
...
fn add_params_and_setup_req_pass_data_for_any_created_data<'a>(pid: PassId, coll_data: &'a mut DataRequestedByPass<'a>, params_and_data: &'a ParamsAndData, p_mappings: &PassMappings)
{
    for param_id in p_mappings.pass_data_req_params.get(pid).into_iter().flatten()
    {
        let param = &params_and_data.params[param_id];
        coll_data.add_req_param(param_id, param)
    }

    for did in p_mappings.pass_data_create_reqs.get(pid).into_iter().flatten()
    {
        coll_data.add_req_create_data(did);
    }
}

fn add_read_write_data_to_req_pass_data<'a>(p_req_res: &'a PassRequiredRes, coll_data: &'a mut DataRequestedByPass<'a>, params_and_data: &'a ParamsAndData)
{
    for (did, req_type) in p_req_res.did_and_req_types.iter()
    {
        match req_type
...
fn upgrade_any_passes_from_waiting_for_res_to_ready_for_sched(p_stages: &mut PassStages, p_res_state: &mut PassResState)
{
    ...
    while passes_moved_to_runnable_on_last_iter
    {
        ...
        for i in p_stages.passes_waiting_for_resources.len()..0
        {
            ...
            if p_res_state.data_usage_state.pass_req_res_are_avail(p_req_res)
            {
                p_res_state.data_usage_state.acquire_req_res_for_pass(p_req_res);
...
impl<'a> DataRequestedByPass<'a>
{
    ...
    pub fn get_data_ref<T: 'static>(&self, d_key: DataId) -> &'a T
    {
        ...
        unsafe
        {
            return Self::get_any_data_from_table_and_downcast(&self.req_data_read, "write data", d_key);
        }  
    }

    pub fn get_data_mut<T: 'static>(&'a mut self, d_key: DataId) -> &'a mut T
    {
        Self::debug_assert_key_exists_in_table(&self.req_data_write, d_key, "data with write access");
        
        unsafe
        {
            return Self::get_any_data_from_table_and_downcast(&self.req_data_read, "read data", d_key)
        }
    }

    pub fn write_data<T: 'static>(&mut self, d_key: DataId, data: T)
    {
        debug_assert!(!self.new_data_created_by_pass.contains_key(d_key),
            "A pass tried writing data ({}) that it did not declare it might write!", d_key);

        self.new_data_created_by_pass.insert(d_key, Some(Box::new(data)));
    }
    ...
    fn debug_assert_key_exists_in_table<T>(table: &HashMap<&str, T>, key: &str, access_type_str: &str)
    {
        debug_assert!(table.contains_key(key),
            format!("A pass tried to access {} ({}) that it did not request! Params requested: {:#?}",
            key, access_type_str, table.keys()));
    }

    unsafe fn get_any_data_from_table_and_downcast<T: 'static>(table: &HashMap<&str, &'a UnsafeCell<Box<dyn Any>>>, type_str: &str, key: &str) -> &'a mut Box<T>
    {
    ...
    fn get_any_param_from_table_and_downcast<T: 'static>(table: &HashMap<ParamId, &'a Box<dyn Any>>, type_str: &str, key: &str) -> &'a T
    {
